...
switch($action)
{
	case 'delete':
		...
		$sql->set_list_tables();
		$sql->delete("vot_onlinesupport","onlinesupport_id",$chon);
		redirect($url);			
		break;			
	...
	case 'updates':
		...
		{
			$values = "onlinesupport_order='".$listOrder[$i]."'";
			$sql = new mysql();
			$sql->update("vot_onlinesupport",$values,"onlinesupport_id='".$listId_O[$i]."'");
		}

		for($i=0;$i<sizeof($listId_V)-1;$i++)
		{
			$values = "onlinesupport_view='".$listView[$i]."'";
			$sql = new mysql();
			$sql->update("vot_onlinesupport",$values,"onlinesupport_id='".$listId_V[$i]."'");
		}
	...
	case 'update':
		...
		*/
		...
		{			
			$values  = "onlinesupport_name='".$nname."',onlinesupport_linkto='".$linkto."',onlinesupport_order='".$order."',onlinesupport_view='".$view."'";
			$sql->update("vot_onlinesupport",$values,"onlinesupport_id=$CURID");					
		}
		elseif($isSave==NULL)
		{
			$fields = "onlinesupport_name,onlinesupport_linkto,onlinesupport_order,onlinesupport_view,language_id";
			$values  = "'".$nname."','".$linkto."','".$order."','".$view."','".$LANG."'";
			$sql->insert("vot_onlinesupport", $fields, $values);
		}
		$isSave = "Y";
	...
	default:
		...
		if($KWD != NULL) 
		{
			$conds .= " AND onlinesupport_name LIKE '%".$KWD."%'";
		}
		if(($orderBy == NULL) || ($orderBy != 'name' && $orderBy != 'order')) 
		{
			$orderBy = "order";
		}
		$others = " ORDER BY onlinesupport_$orderBy $vArrow";
		if($vArrow == 'DESC') 
		{
			$toArrow = NULL;
			if($orderBy == 'order')
		...
		if($tRows > 0)
		{
			...
			while (($sql->set_farray())&&($curRow<=$tRows)&&($curRow<=$curPg*$maxRows))
			{
				...
				if($curRow>$low)
				{
					$nid = $sql->farray["onlinesupport_id"];
					$name = $itemTitle = $sql->farray["onlinesupport_name"];

					if(strlen($name) > $MAXSTRLEN) 
					{
						$name = cutString($name, 0, $MAXSTRLEN)."..";
					}
					$order = $sql->farray["onlinesupport_order"];
					$view = $checked[$sql->farray["onlinesupport_view"]];
					...
					{
						$cur_id = $nid;
						$cur_name = $sql->farray["onlinesupport_name"];
						$linkto = $sql->farray["onlinesupport_linkto"];
						$cur_order = $sql->farray["onlinesupport_order"];
						$cur_view = $checked[$sql->farray["onlinesupport_view"]];
		...
		$rightContentFile = "onlinesupportinfos.php";
		
		include_once("listonlinesupport.php");
		break;
}
?>