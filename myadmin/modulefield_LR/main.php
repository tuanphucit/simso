...
switch($action)
{
	...
	case 'updates':
		...
		for($i=0;$i<sizeof($listId_O)-1;$i++)
		{
			...
			$sql = new mysql();
			$sql->update("$tableblocktypeLR",$values,"homeblocktype_id='".$listId_O[$i]."'");
		}
		...
		for($i=0;$i<sizeof($listId_V)-1;$i++)
		{
			...
			$sql = new mysql();
			$sql->update("$tableblocktypeLR",$values,"homeblocktype_id='".$listId_V[$i]."'");
		}
	...
	case 'update':
		...
		{			
			$values  = "homeblocktype_type='".$type."',homeblocktype_name='".$nname."',homeblocktype_order='".$order."',homeblocktype_view='".$view."'";
			$sql->update("$tableblocktypeLR",$values,"homeblocktype_id=$CURID");					
		}
		elseif($isSave==NULL)
		{
			$fields = "homeblocktype_type,homeblocktype_name,homeblocktype_order,homeblocktype_view";
			$values  = "'".$type."','".$nname."','".$order."','".$view."'";
			$sql->insert("$tableblocktypeLR", $fields, $values);
		}
		$isSave = "Y";
	...
	default:
		...
		{		
			$pageTitle = $opt->optionvalue("$tableblocktypeLR","homeblocktype_name","homeblocktype_id=$PARENTID");
		}
		$pageContent = NULL;
		...
		$sql->set_query("$tableblocktypeLR", "*", $conds, $others);
		$tRows = $sql->nRows;
		...
		if($tRows > 0)
		{
			...
			while (($sql->set_farray())&&($curRow<=$tRows)&&($curRow<=$curPg*$maxRows))
			{
				...
				if($curRow>$low)
				{
					...
					$type = $itemTitle = $sql->farray["homeblocktype_type"];
					$name = $sql->farray["homeblocktype_name"];
					...
					{
						$cur_id = $nid;
						$cur_type = $sql->farray["homeblocktype_type"];
						...
						$type = "<font class=itemselected>$type</font>";
						$sS1 = ""; $sS2 = "";
					}
					else 
					{
					...
					$pageContent .= "<td width=\"250\" $sS2 class=\"rowlist\" title=\"$itemTitle\">$type</td>";
