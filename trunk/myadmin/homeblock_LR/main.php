...
$sql = new mysql;
$opt = new option;
$tableblock_LR="vot_homeblock_lr";
$tableblocktypeLR = "vot_homeblocktype_LR";
...
$linkPath = '<a href="index.php?module=home">'.def_trangchu.'</a> / '.def_cacblockLeftRight;
...
switch($action)
{
	case 'delete':			
		...
		$sql->set_list_tables();
		$sql->delete("$tableblock_LR", "homeblock_id", $chon);
	...
	case 'updates':
		...
		for($i=0;$i<sizeof($listId_O)-1;$i++)
		{
			...
			$sql = new mysql();
			$sql->update("$tableblock_LR",$values,"homeblock_id='".$listId_O[$i]."'");
		}
		...
		for($i=0;$i<sizeof($listId_V)-1;$i++)
		{
			...
			$sql = new mysql();
			$sql->update("$tableblock_LR",$values,"homeblock_id='".$listId_V[$i]."'");
		}
		include_once("includes/create_homeblock.php");				
		redirect($url);
		break;
	...
	case 'update':
		...
		{	
			$values  = "homeblock_name='".$nname."',homeblock_lr='".$pos."',homeblock_pos='".$homepos."',homeblock_order='".$order."',homeblock_view='".$view."'";
			$sql->update("$tableblock_LR", $values, "homeblock_id=$CURID");					
		}
		elseif($isSave == NULL)
		{
			$fields = "homeblock_name,homeblock_lr,homeblock_pos,homeblock_order,homeblock_view,language_id";
			$values  = "'".$nname."','".$pos."','".$homepos."','".$order."','".$view."','".$LANG."'";
			$sql->insert("$tableblock_LR", $fields, $values);
		}		
		$isSave = "Y";
	...
	default:
		...
		$pageTitle = def_cacblockLeftRight;
		$pageContent = NULL;
		...
		$sql->set_query("$tableblock_LR","*",$cond,$other);
		$tRows = $sql->nRows;
		...
		$action = 'edit';
		$and = "PARENTID=$PARENTID&curPg=$curPg&orderBy=$orderBy&vArrow=$vArrow&KWD=$KWD&POS=$POS";
		...
		$homePosSels = $opt->optionselected($homepos, def_chon, "$tableblocktypeLR","homeblocktype_id","homeblocktype_type",$conds);
