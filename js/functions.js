...
function GetDate(lang)
{
	...
	if(lang == '1')
	{
		...
		strDate = listDays[cur_dd] + ", " +  "Ng&#224;y " + cur_d + "/" + cur_m + "/" + cur_y;
	}
	else
	{		
...
var curInfoTop = 0;
var proIsShow = false;
var showTimeInfo = null;
var curInfoLeft = false;
...
{	
	if(!proIsShow)
	{
		if(ns4 || ns6)
		{
			curInfoTop = getObjectById('proInfoControl').offsetTop - 22;
		}
		else
		{
			curInfoTop = getObjectById('proInfoControl').offsetTop;
		}
		if(!curInfoLeft)
		{
			myBrowser = navigator.userAgent;
			myBrowser = myBrowser.toLowerCase();
			if(myBrowser.indexOf('firefox/2') != -1)
			{
				curInfoLeft = getObjectById('proInfoControlArrow').offsetLeft - 135;
				//curInfoLeft = getObjectById('productInfo').offsetLeft + 130;
				getObjectById('productInfo').style.left = curInfoLeft+"px";
			}
			else if(myBrowser.indexOf('firefox/3') != -1)
			{
				//curInfoLeft = getObjectById('MB_window').offsetLeft;
				curInfoLeft = getObjectById('proInfoControlArrow').offsetLeft - 135;
				getObjectById('productInfo').style.left = curInfoLeft+"px";
			}
			else
			{
				//curInfoLeft = getObjectById('proInfoControlArrow').offsetLeft - 295;
				curInfoLeft = getObjectById('productInfo').offsetLeft - 150;
				getObjectById('productInfo').style.left = curInfoLeft+"px";
			}
			curInfoLeft = true;
		}
		getObjectById('productInfo').style.display = '';
		showProductInfo();
	}
	else
	{
...
{
	if(curInfoHeight < proInfoMaxH)
	{
		curInfoHeight += 10;
		curInfoTop -= 10;
		getObjectById('productInfo').style.top = curInfoTop+"px";
		getObjectById('productInfo').style.height = curInfoHeight+"px";
		showTimeInfo = setTimeout("showProductInfo()", 1);
	}
	else
	{
		clearTimeout(showTimeInfo);
		getObjectById('proInfoControlArrow').src = _img_dir.concat('/down_arrow.jpg');
		proIsShow = true;
	}
}
function hideProductInfo()
{
	if(curInfoHeight > 10)
	{
		...
		getObjectById('productInfo').style.top = curInfoTop+"px";
		getObjectById('productInfo').style.height = curInfoHeight+"px";
		showTimeInfo = setTimeout("hideProductInfo()", 1);
	}
	else
	{
		clearTimeout(showTimeInfo);
		curInfoHeight -= 10;
		curInfoTop += 10;
		getObjectById('productInfo').style.display = 'none';
		getObjectById('proInfoControlArrow').src = _img_dir.concat('/up_arrow.jpg');
		proIsShow = false;
	}
}