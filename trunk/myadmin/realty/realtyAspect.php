...
switch($doAction)
{
	case 'del':
		$sql->set_list_tables();
		$sql->delete("vot_realtyaspect","realtyaspect_id",$id);
	...
	case 'update':
		...
		{			
			$values  = "realtyaspect_name='".$nname."',realtyaspect_order='".$order."',realtyaspect_view='".$view."',modules_id='".$CURMOD."'";
			$sql->update("vot_realtyaspect",$values,"realtyaspect_id=$id");					
		}
		elseif($isSave==NULL)
		{
			$fields = "realtyaspect_name,realtyaspect_order,realtyaspect_view,language_id,modules_id";
			$values  = "'".$nname."','".$order."','".$view."','".$LANG."','".$CURMOD."'";
			$sql->insert("vot_realtyaspect", $fields, $values);
		}
		$isSave = "Y";
	...
	default:
		...
		$conds = "realtyaspect_id='".$id."'";
		$sql->set_query("vot_realtyaspect", "*", $conds);
		if($sql->set_farray())
		{
			$itemName = $sql->farray["realtyaspect_name"];
			$itemOrder = $sql->farray["realtyaspect_order"];
		}
?>
<html>
<head>
