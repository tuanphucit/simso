...
switch($doAction)
{
	case 'del':
		$sql->set_list_tables();
		$sql->delete("vot_documentarea","documentarea_id",$id);
	...
	case 'update':
		...
		{			
			$values  = "documentarea_name='".$nname."',documentarea_order='".$order."',documentarea_view='".$view."',modules_id='".$CURMOD."'";
			$sql->update("vot_documentarea",$values,"documentarea_id=$id");					
		}
		elseif($isSave==NULL)
		{
			$fields = "documentarea_name,documentarea_order,documentarea_view,language_id,modules_id";
			$values  = "'".$nname."','".$order."','".$view."','".$LANG."','".$CURMOD."'";
			$sql->insert("vot_documentarea", $fields, $values);
		}
		$isSave = "Y";
	...
	default:
		...
		$pageTitle = "L&#297;nh v&#7921;c v&#259;n b&#7843;n";

		$conds = "documentarea_id='".$id."'";
		$sql->set_query("vot_documentarea", "*", $conds);
		if($sql->set_farray())
		{
			$itemName = $sql->farray["documentarea_name"];
			$itemOrder = $sql->farray["documentarea_order"];
		}
?>
<html>
<head>
