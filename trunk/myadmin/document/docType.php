...
switch($doAction)
{
	case 'del':
		$sql->set_list_tables();
		$sql->delete("vot_documenttype","documenttype_id",$id);
	...
	case 'update':
		...
		{			
			$values  = "documenttype_name='".$nname."',documenttype_order='".$order."',documenttype_view='".$view."',modules_id='".$CURMOD."'";
			$sql->update("vot_documenttype",$values,"documenttype_id=$id");					
		}
		elseif($isSave==NULL)
		{
			$fields = "documenttype_name,documenttype_order,documenttype_view,language_id,modules_id";
			$values  = "'".$nname."','".$order."','".$view."','".$LANG."','".$CURMOD."'";
			$sql->insert("vot_documenttype", $fields, $values);
		}
		$isSave = "Y";
	...
	default:
		...
		$conds = "documenttype_id='".$id."'";
		$sql->set_query("vot_documenttype", "*", $conds);
		if($sql->set_farray())
		{
			$itemName = $sql->farray["documenttype_name"];
			$itemOrder = $sql->farray["documenttype_order"];
		}
?>
<html>
<head>
